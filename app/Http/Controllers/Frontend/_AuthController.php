...
class AuthController extends Controller
{
    ...
    public function requestOtp(Request $request)
    {
        ...
        // return redirect()->route('member.verify')->with('success','our OTP has been sent successfully.');
        $otp_msg = "เราส่งรหัส OTP ไปยังหมายเลข : $phone";
        return view('frontend.verify_otp', compact('phone','otp_msg'));
    }
    ...
    public function verifyOtp(Request $request)
    {
        ...
        if (!$member) {
            return back()->withErrors(['phone' => 'ไม่พบผู้ใช้งาน']);
        }

        // login ด้วย guard member
        Auth::guard('member')->login($member);

        // if(Auth::guard('member')->attempt($request->phone)){
    ...
    public function requestOtp2(Request $request)
    {
        ...
        $result = $this->otpService->requestOtp($request->phone);

        dd($result);

        if ($result && isset($result['token'])) {
            // redirect ไปหน้า verify พร้อม token
            return redirect()->route('otp.verify.form', ['token' => $result['token']]);
        }
    ...
    public function verifyOtp2(Request $request)
    {
        ...
        $result = $this->otpService->verifyOtp($request->token, $request->otp_code);
